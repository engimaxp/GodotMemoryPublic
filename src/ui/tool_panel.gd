...
@onready var add = %Add
const tool_row = preload("res://src/ui/tool_row.tscn")
...
func _ready():
	popup_tool_edit = preload("res://src/ui/popup_tool_edit_menu.tscn").instantiate()
	get_tree().current_scene.call_deferred("add_child",popup_tool_edit)
	popup_tool_edit.hide()
	popup_tool_edit.refresh.connect(_load_panel)
...
func build_condition_from()->Array:
	...
	if search_logic.search_conditions_dic.is_empty():
		...
		result[0] = "(Name like '%%%s%%' or Desc like '%%%s%%' or Link like '%%%s%%') and IsDelete = 0" % [s,s,s]
...
func _load_panel():
	for c in v_box_container.get_children():
		c.queue_free()
	var total_result = Db.QueryAllCountFromDb("Tool")
	var tools
	if not search_logic.empty_condition():
		var condition = build_condition_from()
		var query_size = Db.QueryWithConditionCount(condition[0],condition[1],Constants.TagType.Tool)
		tools = Db.QueryWithConditionWithPage(condition[0],condition[1],Constants.TagType.Tool,page,true)
		found_num_label.text = "(%d/%d)" % [query_size,total_result]
		page_container.total_page = ceil(float(query_size) / float(page["size"]))
	else:
		tools = Db.QueryAllWithFromDBwithPage("Tool",page,true)
		found_num_label.text = "(%d/%d)" % [total_result,total_result]
		page_container.total_page = ceil(float(total_result) / float(page["size"]))
	for tool in tools:
		var e = tool_row.instantiate()
		e.tool_info = tool
...
func edit_pressed(dic,row):
	popup_tool_edit.tool_info = dic
	popup_tool_edit.show()
...
func _on_swap_display_pressed():
	get_tree().call_group("ToolRow","swap_display")
